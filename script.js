/* script.js — logica dell'app Controller Map Art Generator. Tutto avviene lato client. */
...
(() => {
  ...
  let result = null;         // { width, height, grid } dell'ultima generazione confermata

  // stato del ritaglio, in coordinate dell'immagine originale
  const cropState = { mode: 'cover', cx: 0, cy: 0, zoom: 1 };
  let dragging = false;
  let dragStart = null;
  ...
  function computeCoverBox(imgW, imgH, aspect) {
    let boxW, boxH;
    if (imgW / imgH > aspect) { boxH = imgH; boxW = imgH * aspect; }
    else { boxW = imgW; boxH = imgW / aspect; }
    return { boxW, boxH };
  }
  ...
  function currentCropRect() {
    ...
    return { x: center.cx - boxW / 2, y: center.cy - boxH / 2, w: boxW, h: boxH };
  }
  ...
  // quando cambia la dimensione target cambia anche il rapporto d'aspetto
  ...
  cropCanvas.addEventListener('pointerdown', e => {
    ...
    dragging = true;
    dragStart = { x: e.clientX, y: e.clientY, cx: cropState.cx, cy: cropState.cy };
  ...
  function drawCropper() {
    ...
    if (cropState.mode !== 'cover') return; // niente riquadro per adatta/deforma: si usa l'intera immagine
    ...
    // oscura tutto tranne il riquadro di ritaglio
  ...
  /** Disegna l'immagine sorgente in un canvas di destinazione WxH, rispettando la modalità di adattamento scelta. */
  function paintSourceRegion(ctx, width, height) {
    ...
    // cover (default): usa il riquadro di ritaglio corrente
    const rect = currentCropRect();
    ctx.drawImage(currentImage, rect.x, rect.y, rect.w, rect.h, 0, 0, width, height);
  }

  // ============================================================
  // 3. Modalità e dimensione
  // ============================================================
  ...
  // ============================================================
  // 5. Generazione anteprima completa
  // ============================================================
...
`;
  ...
  // ============================================================
  // 8. Cronologia (carosello in cima alla pagina)
  // ============================================================
  ...
  async function refreshHistoryCarousel() {
    ...
    try {
      ({ shared, entries } = await CMAGHistory.listEntries());
    } catch (err) {
    ...
    if (!entries.length) {
      ...
      historyEmpty.textContent = shared
        ? 'Ancora nessuna build salvata: carica una foto e sii il primo a comparire qui.'
        : 'Ancora nessuna build salvata in questo browser: carica una foto per iniziare.';
      return;
    }
    ...
    // duplica la sequenza per uno scorrimento continuo senza scatti
